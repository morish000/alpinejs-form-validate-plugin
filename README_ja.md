...
[HTMX](https://htmx.org/)の[hx-post](https://htmx.org/attributes/hx-post/)と[Validation統合](https://htmx.org/docs/#validation)を使いたかったのですが、HTML5 標準バリデーションを使った便利なユーティリティが見つからなかったため自作しました。  
...
- `required`属性が付いていない要素では未入力・未選択の場合は他のバリデーションは動かず、正常終了になります。これはHTML5 標準バリデーションの動作に従っています。
...
- `フィールドタグのディレクティブに指定した値` > `フォームタグの決定値` > `初期化時に指定した値` > `デフォルト値`
